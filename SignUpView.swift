...
struct SignUpView: View {
    ...
    private func handleSignUp() {
        ...
        if UserManager.shared.userExists(email: email) {
            alertMessage = "An account with this email already exists. Please sign in instead."
            showAlert = true
            return
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            if UserManager.shared.registerUser(email: email, name: name, password: password) {
                isLoading = false
                onSignUp?(email, name)
                onNavigate?("dashboard")
            } else {
                isLoading = false
                alertMessage = "Failed to create account. Please check your information and try again."
                showAlert = true
            }
        }
    }
