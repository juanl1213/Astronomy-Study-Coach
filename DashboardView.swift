...
struct DashboardView: View {
    ...
    // Progress data from ProgressManager
    ...
    // Computed study progress from topic progress
    private var studyProgress: [(String, Double)] {
        topicProgress.prefix(4).map { topic in
            (topic.name, Double(topic.progress) / 100.0)
        }
    }
    
    // Computed quick stats from overall progress
    private var quickStats: [(String, String, String)] {
        let progress = overallProgress
        return [
            ("Total Study Time", formatStudyTime(progress.studyTime), "book.fill"),
            ("Quizzes Completed", "\(progress.completedQuizzes)", "target"),
            ("Current Streak", "\(progress.currentStreak) days", "flame.fill"),
            ("Topics Mastered", "\(progress.completedTopics)/\(progress.totalTopics)", "trophy.fill")
        ]
    }
    
    // Sample achievements (can be enhanced later with real achievement system)
    private var recentAchievements: [(String, String, String)] {
        // For now, return sample data. Can be enhanced with real achievements later
        return [
