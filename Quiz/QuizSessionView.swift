...
//
//  Quiz session view for taking quizzes
//
...
struct QuizSessionView: View {
    ...
    @State private var quizResultSaved = false
    
    // Load questions based on quizId, or use demo questions
    private var questions: [QuizQuestion] {
        if let quizId = quizId, let quizQuestions = QuizQuestions.allQuizzes[quizId] {
    ...
    var body: some View {
        ...
        .id(quizId ?? "demo") // Reset scroll position when quiz changes
        .onAppear {
        ...
        .onChange(of: quizComplete) { newValue in
            // Save quiz result when quiz completes
            if newValue && !quizResultSaved {
                saveQuizResult()
            }
        }
    }
    ...
    private func saveQuizResult() {
        guard !quizResultSaved else { return }
        
        let score = calculateScore()
        let timeSpent = quizStartTime.map { Int(Date().timeIntervalSince($0)) } ?? 0
        let quizIdToSave = quizId ?? "unknown"
        
        ProgressManager.shared.saveQuizResult(
        ...
        // Also track study time (convert seconds to minutes)
        if timeSpent > 0 {
    ...
    private func resetQuiz() {
        ...
        answers = []
        quizComplete = false
        quizStartTime = Date() // Reset start time for retake
        quizResultSaved = false // Allow saving result again on retake
    }
...
#Preview {
    QuizSessionView(isActive: .constant(true), quizId: "solar-system")
